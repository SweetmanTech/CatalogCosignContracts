    ...
    MulticallV2
{
    ...
    event TokenCreated(
        ...
        string _uri,
        bytes32 indexed _contentHash
    );
    event TokenPayoutAddressUpdated(
        uint256 indexed tokenId,
        address indexed payoutAddress
    );
    event ReferralRewardUpdated(
        uint256 indexed tokenId,
        uint256 indexed rewardBPS
    );
    ...
    modifier onlyAdmin() {
        if (
            msg.sender != owner() && authPermissions[msg.sender] != AUTH_ADMIN
        ) {
            revert Unauthorized();
        }
        _;
    ...
    modifier onlyArtistOrAdmin(uint256 _tokenId) {
        ...
        if (
            msg.sender != tokenData.artist &&
            authPermissions[msg.sender] != AUTH_ADMIN &&
            msg.sender != owner()
        ) {
            revert Unauthorized();
        }
    ...
    ) external nonReentrant onlyArtistOrAdmin(_tokenId) {
    ...
    function setReferralReward(
        uint256 _tokenId,
        uint256 _rewardBPS
    ) external onlyArtistOrAdmin(_tokenId) {
        if (_tokenId == CONTRACT_BASE_ID) {
            revert InvalidTokenId();
        }
    ...
    function setupToken(
        InputTokenData calldata _input,
        uint256 _referralRewardBPS
    ) external onlyAdmin returns (uint256 tokenId) {
        if (
            _input.artistPayoutAddress == address(0) ||
            _input.artist == address(0)
        ) {
            revert InvalidAddress();
        }
    ...
    function updateTokenArtistPayoutAddress(
        uint256 _tokenId,
        address _artistPayoutAddress
    ) external onlyArtistOrAdmin(_tokenId) {
        if (_tokenId == CONTRACT_BASE_ID) {
            revert InvalidTokenId();
        }
    ...
    function _handlePayment(
        ...
        if (referralReward > 0) {
            ...
            if (_ref0 != address(0) && _ref0 != tokenData.artistPayoutAddress) {
                uint256 ref0Share = _ref1 == address(0) ||
                    _ref1 == tokenData.artistPayoutAddress
                    ? referralShare
                    : referralShare / 2;
    ...
    /// @return TokenData the TokenData struct
    function tokenInfoByRecord(
        uint256 _L1TokenId,
        address _L1ContractAddress
    ) external view returns (TokenData memory) {
